...
function AddGrid(id) {
    ...
    f!=undefined && f["btn_text"]!=undefined ? btntext=f["btn_text"] : btntext="Filtruj";//defines button text
    ...
    if(fltgrid) {
        ...
        for(var i = 0; i < n; i++) { // adds filters:
            var fltcell = fltrow.insertCell(i);
            //fltcell.noWrap = true;
            i == n-1 && displayBtn == true ? inpclass = "flt_s form-control" : inpclass = "flt form-control";
            ...
            if (f == undefined || f["col_"+i] == undefined || f["col_"+i]=="none") {
                ...
                // Ukrycie pola filtracji:
                // TODO: Poprawić sztywne warunki;
                if ((id === "resourceTable" && (i == n-1 || i == 0)) || (id === "commentsTable" && i == 0)
                        || (id === "usersTable") && (i == n-1)) {
                    inptype = "hidden";
                }
                ...
                // Zbudowanie pola:
                // TODO: Poprawić sztywne warunki;
                if ((id === "resourceTable" && (i == 1 || i == 2))
                        || (id === "usersTable" && i == 5) || (id === "commentsTable" && i == 4)) { // Select:
                    ...
                    var scriptParams = document.getElementById('filterScript');
                    var message = scriptParams.getAttribute('data-input-text');
                    var inp = createElm("input", ["id", "flt" + i + "_" + id], ["type", inptype], ["class", inpclass],
                            ["placeholder", message], ["style", "margin-right:5%"]);
                    inp.className = inpclass;// for ie<=6
                    fltcell.appendChild(inp);
                    if (enterkey) {
                        inp.onkeyup = DetectKey;
                    }
                }
            }
        }
...
{
    var scriptParams = document.getElementById('filterScript');
    var message = scriptParams.getAttribute('data-select-text');
    var t = grabEBI(id);
    ...
    var currOpt = new Option(message,"",false,false); //1st option
